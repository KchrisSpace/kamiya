...
### 9. 优惠券实体 (user_coupons)

**实体描述**: 存储系统中以折扣码形式存在的优惠券记录，包括所属用户、来源订单、折扣比例及使用状态等。

**属性列表**:

- **id** (主键, Number) - 优惠券唯一标识  
- **user_id** (外键, String) - 关联用户 ID → users.id  
- **order_id** (外键, Number, 可为空) - 来源订单 ID → normal_orders.id（分享订单赠券时记录）  
- **code** (唯一, String) - 折扣码字符串（字母+数字随机组合）  
- **discount_rate** (Number) - 折扣百分比（如 90 表示 9 折）  
- **status** (String) - 状态（unused/used/expired）  
- **created_at** (String) - 发放时间  
- **used_at** (String, 可为空) - 使用时间  
- **expired_at** (String, 可为空) - 过期或强制失效时间  
- **updated_at** (String) - 最近更新时间  

**关系**:

- 多对 1 → 用户实体 (users) [通过 user_id]  
- 多对 1 → 订单实体 (normal_orders) [通过 order_id，可为空]
...
9. **用户 (users) → 优惠券 (user_coupons)**

   - 关系类型: 1 对多 (1:N)  
   - 关系描述: 一个用户可以拥有多条不同来源的优惠券记录  
   - 外键: user_coupons.user_id → users.id

10. **订单 (normal_orders) → 优惠券 (user_coupons)**

   - 关系类型: 1 对多 (1:N)  
   - 关系描述: 部分优惠券由分享某一笔订单生成，一个订单可以产生多条关联的优惠券记录  
   - 外键: user_coupons.order_id → normal_orders.id（可为空）
