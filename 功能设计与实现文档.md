...
const addItem = async (productId, quantity = 1, userId = null) => {
  try {
    ...
    if (newQuantity > currentStock) {
      throw new Error(
        `库存不足，当前库存为 ${currentStock}，您已选择 ${existingItem ? existingItem.quantity : 0} 件，最多只能再添加 ${currentStock - (existingItem ? existingItem.quantity : 0)} 件`,
      );
    }
  ...
  try {
    ...
    for (const item of cartStore.cartItems) {
      ...
      if (currentStock < item.quantity) {
        throw new Error(
          `商品 ${product.title} 库存不足，当前库存为 ${currentStock}，您选择了 ${item.quantity} 件`,
        );
      }
    }
...
折扣与优惠券模块用于提升用户复购率和黏性，为商家提供灵活的促销与会员运营能力。系统支持三类核心能力：  
（1）优惠券发放：包括用户在前台“分享订单”后系统自动赠送优惠券，以及管理员在后台根据客户 ID 主动发放优惠券；  
（2）折扣码使用：用户在创建订单时输入折扣码，系统校验有效性后按折扣比例重新计算应付金额；  
（3）优惠券管理：前台用户可在个人中心查看“我的优惠券”，后台管理员在“折扣码管理”页面集中查看、筛选、发放及强制失效折扣码。

### （2）实现方案

1. 前台分享订单赠券  
   用户在“我的订单”页面中，对状态为“已完成”的订单点击“分享订单领优惠券”按钮。前端弹出确认框，确认后：  
   1）生成随机折扣码（字母+数字），默认折扣比例为 9 折（`discount_rate = 90`）；  
   2）通过 `POST /user_coupons` 向 `user_coupons` 表写入记录，包含 `user_id`、`order_id`、`code`、`discount_rate`、`status=unused`、`created_at` 等字段；  
   3）调用 `PATCH /normal_orders/:id` 将订单标记为 `has_shared=true`，防止重复领取；  
   4）使用 `html2canvas` 对订单卡片进行截图，生成预览弹窗，用户点击“下载图片并完成分享”后将截图保存到本地，并弹出“图片已下载，分享完成”的提示。

2. 前台创建订单时使用折扣码  
   在“创建订单”页面中，系统在应付金额上方增加“折扣码”输入框和“使用”按钮。用户输入折扣码并点击“使用”后：  
   1）前端调用 `GET /user_coupons?user_id=:id&code=:code&status=unused` 校验折扣码是否存在且为未使用状态；  
   2）校验通过时，记录当前优惠券对象到 `appliedCoupon`，并通过计算属性将原始总价 `totalPrice` 与折扣后应付金额 `finalPayable` 区分开；  
   3）创建订单（普通订单或定制订单）时，提交数据中的 `total_price` 使用 `finalPayable`，确保订单金额与折扣展示一致；  
   4）订单创建成功后，若使用了优惠券，则调用 `PATCH /user_coupons/:id` 将该券状态更新为 `used`，并记录 `used_at` 与 `updated_at` 时间。

3. 前台“我的优惠券”展示  
   在个人中心页面中新增“我的优惠券”板块。页面加载时调用 `GET /user_coupons?user_id=:id` 获取当前用户所有优惠券，并按 `created_at` 倒序排序。前端根据 `status` 字段以不同样式展示：  
   （1）`unused`：标记为“未使用”，背景高亮，并计入“可用优惠券数量”；  
   （2）`used`：标记为“已使用”，显示使用时间；  
   （3）`expired`：标记为“已过期”，显示过期时间。  
   用户点击“我的优惠券”统计卡片可展开或收起优惠券列表。

4. 后台折扣码管理  
   在后台管理系统中新增“折扣码管理”菜单和页面，路由为 `/admin/coupons`。页面进入时调用 `GET /user_coupons` 获取所有折扣码数据，支持按照用户 ID、折扣码、状态进行筛选。顶部提供“发放折扣码”区域：  
   （1）管理员输入客户 ID 和折扣百分比（1–100，例如 90 表示 9 折）；  
   （2）前端生成随机折扣码字符串，通过 `POST /user_coupons` 新增记录，`status` 设置为 `unused`；  
   （3）操作成功后自动刷新下方表格。  
   表格每行提供“强制失效”操作按钮：对于状态为 `unused` 的折扣码，管理员可以点击该按钮调用 `PATCH /user_coupons/:id` 将状态更新为 `expired`，同时写入 `expired_at` 和 `updated_at`，以防止该折扣码继续被使用。
...
| 接口名称 | 用户分享订单赠券接口                                          |
| -------- | ------------------------------------------------------------- |
| 接口描述 | 用户分享已完成订单后，系统自动生成折扣码并发放给用户          |
| URL      | `{{baseurl}}/user_coupons`                                    |
| method   | POST                                                          |
| 请求参数 | user_id、order_id、code、discount_rate、status、created_at 等 |
| 返回参数 | 创建成功的优惠券记录                                          |

| 接口名称 | 用户查询个人优惠券接口                 |
| -------- | -------------------------------------- |
| 接口描述 | 用户在个人中心查看自己全部优惠券       |
| URL      | `{{baseurl}}/user_coupons?user_id=:id` |
| method   | GET                                    |
| 请求参数 | user_id                                |
| 返回参数 | 当前用户的优惠券数组                   |
...
| 接口名称 | 订单创建时折扣码校验接口                                        |
| -------- | --------------------------------------------------------------- |
| 接口描述 | 在创建订单前验证折扣码是否有效且未使用                          |
| URL      | `{{baseurl}}/user_coupons?user_id=:id&code=:code&status=unused` |
| method   | GET                                                             |
| 请求参数 | user_id、code、status                                           |
| 返回参数 | 符合条件的优惠券数组（通常为 0 或 1 条）                        |

| 接口名称 | 后台发放折扣码接口                                         |
| -------- | ---------------------------------------------------------- |
| 接口描述 | 管理员根据客户 ID 主动发放折扣码                           |
| URL      | `{{baseurl}}/user_coupons`                                 |
| method   | POST                                                       |
| 请求参数 | user_id、code、discount_rate、status=unused、created_at 等 |
| 返回参数 | 创建成功的优惠券记录                                       |

| 接口名称 | 折扣码状态更新接口                         |
| -------- | ------------------------------------------ |
| 接口描述 | 在用户使用或管理员强制失效时更新折扣码状态 |
| URL      | `{{baseurl}}/user_coupons/:id`             |
| method   | PATCH                                      |
| 请求参数 | status、used_at 或 expired_at、updated_at  |
| 返回参数 | 更新后的优惠券记录                         |
